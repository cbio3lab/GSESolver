...
data$GSESolver_binary <- ifelse(data$`GSESolver prediction 2`=='Use GSE', 0, 1)
...
#correlation plot
p <- ggplot(data) +
  geom_point(aes(y=logS,x=logS_GSE,colour=Type, shape = `GSESolver prediction 2`),size=4,alpha=0.7) +
  ...
  scale_shape_manual("GSESolver prediction",values = c(17,19)) + 
...
p <- ggplot(data = data ) + 
  ...
  geom_point(aes(x = logS_GSE, y = res, color = Type, shape = `GSESolver prediction 2`), 
             size = 4, alpha = 0.8) + 
...
data_descs$GSESolver.prediction <- ifelse(data_descs$dev=='NO','Use GSE','Do not use GSE')
data_descs$Type <- "SolCbio3Database"
data_descs <- data_descs %>% select(-c(X,dev))
...
p <- ggplot() +
  ...
  scale_color_manual("SolCbio3Database", values = c("gray50","blue4")) + 
  geom_point(
    data = apps,
